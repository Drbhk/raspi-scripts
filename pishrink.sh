...
LOOPD=$(losetup -f)
P2=p2
sudo losetup -d $LOOPD || echo "$LOOPD is free"
sudo losetup $LOOPD $1
sudo partprobe $LOOPD
sudo lsblk $LOOPD
sudo e2fsck -f $LOOPD$P2
sudo resize2fs -p $LOOPD$P2 -M
sudo dumpe2fs -h $LOOPD$P2 | tee /tmp/dumpe2fs
...
sed -e 's/\s*\([\+0-9a-zA-Z]*\).*/\1/' << EOF | sudo fdisk $LOOPD || echo "Ignore that error."
...
sudo mount $LOOPD$P2 /tmp/mountpoint
sudo dd if=/dev/zero of=/tmp/mountpoint/zero.txt conv=fsync oflag=direct bs=512k status=progress || echo "Expected to fail with out of space"
